...
class SentimentAnalysis:
    ...
    def plotPieChart(self, positive, wpositive, spositive, negative, wnegative, snegative, neutral, searchTerm,
                     noOfSearchTerms):
        labels = ['Strongly Positive [' + str(spositive) + '%]',
        ...
        patches, texts = pyplot.pie(sizes, colors=colors, startangle=90)
        pyplot.legend(patches, labels, loc="best")
        pyplot.title('How people are reacting on ' + searchTerm + ' by analyzing ' + str(noOfSearchTerms) + ' Tweets.')
        pyplot.axis('equal')
        pyplot.tight_layout()
        pyplot.show()
